...
case 'upload':
 ...
 if($imageCheck){
  $message = '<p class="red">An image by that name already exists.</p>';
 } elseif (empty($invId) || empty($imgName)) {
  $message = '<p class="red">You must select a product and image file for the product.</p>';
 } else {
  ...
  if ($result) {
   $message = '<p class="green">BAM! The upload succeeded.</p>';
  } else {
   $message = '<p class="red">SPLAT! The upload failed.</p>';
  }
 }
...
case 'delete':
  ...
  if ($remove) {
   $message = "<p class='green'>BAM! $filename was successfully deleted.</p>";
  } else {
   $message = "<p class='red'>SPLAT! $filename was NOT deleted.</p>";
  }
...
default:
  ...
  if (count($imageArray)) {
   ...
   $imageDisplay = '<p class="red">Sorry, no images could be found.</p>';
  }
